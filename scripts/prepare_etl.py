...
def process_record(args):
    """Worker function to process and save a single ETL record."""
    _bytes, i, mode, record_size, file_stem, img_dir = args
    try:
        ...
        img_filename = f"{file_stem}_{i:06d}.png"
        ...
        print(f"Error processing record {i} of {file_stem}: {e}")
        return None

def extract_etl(file_path: Path, output_dir: Path):
    """Extract all records from ETL binary file in parallel."""
    ...
    # Read all raw records into memory
    records = []
    with open(file_path, "rb") as f:
        for _ in range(num_records):
            ...
            if len(_bytes) < record_size:
                break
            records.append(_bytes)
            
    # Prepare arguments for multiprocessing pool
    pool_args = [
        (records[i], i, mode, record_size, file_path.stem, img_dir)
        for i in range(len(records))
    ]
    
    # Run extraction in parallel using a Process Pool
    samples = []
    num_workers = min(mp.cpu_count(), 8)  # Cap workers to avoid disk thrashing
    
    with mp.Pool(processes=num_workers) as pool:
        # Use tqdm to track progress over the process pool execution
        for res in tqdm(pool.imap_unordered(process_record, pool_args), total=len(pool_args), desc=f"  {file_path.name}", leave=False):
            if res is not None:
                samples.append(res)
