...
# Parse arguments to support --train or --train-only flags
TRAIN_ONLY=false
MAIN_ARGS=()
for arg in "$@"; do
...
if [ "$TRAIN_ONLY" = "false" ]; then
    ...
    echo "⚡ Skipping setup & dataset generation. Going straight to training..."
    if [ -f "venv/bin/activate" ]; then
        source venv/bin/activate
    else
        echo "❌ Error: Virtual environment 'venv' not found. Please run './setup_and_train.sh' without --train once first to initialize."
        exit 1
    fi
fi
